...
if __name__ == "__main__":
    ...
    for con in CONFIG:
        ...
        df = transformar(df)
        df.to_excel(f'out/data_limpia{adcp}.xlsx')
        ...
        fig_hs_rose_wr, ax_hs_wr = plt.subplots(figsize=(8, 6))
        ...
        fig_tp_rose_wr, ax_tp_wr = plt.subplots(figsize=(8, 6))
        ...
        with PdfPages(pdf_path) as pdf:
            ...
            try:
                ...
                fig_hs_title.clf()
                hs_title_text = f"HS — {report_title}"
                fig_hs_title.text(0.5, 0.6, hs_title_text, ha='center', va='center', fontsize=22, weight='bold')
                fig_hs_title.text(0.5, 0.45, f"ADCP: {adcp}", ha='center', va='center', fontsize=12)
            ...
            # Save HS figures
            for fig_obj in [fig_hs_rose, fig_hs_rose_wr, fig_hs_bars, fig_hs_table, fig_hs_ts]:
                try:
            ...
            try:
                ...
                fig_tp_title.clf()
                tp_title_text = f"TP — {report_title}"
                fig_tp_title.text(0.5, 0.6, tp_title_text, ha='center', va='center', fontsize=22, weight='bold')
                fig_tp_title.text(0.5, 0.45, f"ADCP: {adcp}", ha='center', va='center', fontsize=12)
            ...
            # Save TP figures
            for fig_obj in [fig_tp_rose, fig_tp_rose_wr, fig_tp_bars, fig_tp_table, fig_tp_ts]:
                try:
                    fig_obj.tight_layout()
                except Exception:
                    pass
